...
module.exports.getComment = async (req, res) => {
    ...
    try {
        const comment = await Comment.find({ postId }).populate('userId', 'name profilePic _id fileType').sort({ createdAt: -1 });
...
module.exports.deleteComment = async (req, res) => {
    ...
    try {
        if (!commentId) {
            return res.staus(404).json({ message: 'No Comment Id Found' })
        }
